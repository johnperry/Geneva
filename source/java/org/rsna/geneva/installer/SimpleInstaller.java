...
public class SimpleInstaller extends JFrame {
	...
	private File getDirectory() {
		if (chooser == null) {
			File currentDirectory = findStartingDirectory();
	...
	private File findStartingDirectory() {
		...
		if (javaPrograms.exists() && javaPrograms.isDirectory()) return javaPrograms;

		javaPrograms = new File(currentDirectory, "Java-Programs");
		if (javaPrograms.exists() && javaPrograms.isDirectory()) return javaPrograms;

		javaPrograms = new File(currentDirectory, "Java_Programs");
		if (javaPrograms.exists() && javaPrograms.isDirectory()) return javaPrograms;
	...
	private void positionFrame() {
		...
		setSize(scr.width * 2/5, scr.height /3);
		int x = (scr.width - getSize().width) /4;
		int y = (scr.height - getSize().height) /4;
